...
int main(){
    ...
    vector<mint998244353> f(n+10); // f[i]:=(1~iの数からいくつか選ぶ際，隣り合う2数のうち少なくとも一つ選ぶ方法の通り数)．
    f[1]=2, f[2]=3;
    FOR(i,3,n+1) f[i]=f[i-1]+f[i-2];
    
    vector<mint998244353> g(n+10); // g[i]:=(i頂点のノンサイクルの辺の部分集合であって，辺被覆であるものの通り数)．
    g[1]=1, g[2]=3, g[3]=4;
    FOR(i,4,n+1) g[i]=f[i-1]+f[i-3];
